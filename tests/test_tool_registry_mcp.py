...
def test_tool_registry_exposes_mcp_readonly_tools():
    ...
    response = client.get("/api/tools/registry?role=analyst")
...
def test_tool_registry_enforces_role_permissions():
    ...
    response = client.post(
        ...
        json={
            ...
            "arguments": {"order_id": "53cdb2fc8bc7dce0b6741e2150273451"},
            "role": "viewer",
        },
    )
    assert response.status_code == 200
...
def test_mcp_lite_tools_list_and_call():
    client = TestClient(main.app)
    listed = client.post("/api/mcp", json={"jsonrpc": "2.0", "id": "list-1", "method": "tools/list", "params": {}})
...
def test_function_agent_reuses_order_id_for_followup_tool_selection():
    ...
    try:
        ...
        first = main.orchestrator.function_agent.respond("查询订单 53cdb2fc8bc7dce0b6741e2150273451 的物流状态", session_id=session_id)
    ...
    assert second["tool_trace"][0]["arguments"]["order_id"] == "53cdb2fc8bc7dce0b6741e2150273451"
