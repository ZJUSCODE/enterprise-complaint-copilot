...
def _final_sse_event(body: str) -> dict:
    for block in body.strip().split("\n\n"):
        ...
        return json.loads("\n".join(data_lines))
    raise AssertionError("stream did not include a final event")


def test_stream_chat_multi_agent_preserves_multi_agent_output():
    client = TestClient(main.app)
    response = client.post(
        "/api/chat/stream",
        json={
            "message": "query refund compensation policy risk",
            "mode": "multi_agent",
    ...
    payload = _final_sse_event(response.text)
    assert "data" not in payload
    assert payload["mode"] == "multi_agent"
    assert payload["agent_dispatch"]


def test_stream_chat_guardrail_includes_review_case_and_runtime_fields():
    client = TestClient(main.app)
    response = client.post(
        "/api/chat/stream",
        json={"message": "delete order and approve refund", "mode": "function_call_agent"},
    )
...
def test_sql_rag_chain_carries_rag_token_usage_to_audit_shape():
    ...
    assert "结论：" in payload["summary"]
    assert "金额阈值本身" in payload["highlights"][-1]
    assert "随后已用 SQL 摘要检索" not in payload["summary"]
    rag_step = next(item for item in payload["tool_trace"] if item["tool"] == "langchain_rag")
    assert rag_step["token_usage"]["total_tokens"] == payload["token_usage"]["total_tokens"]


def test_new_business_tool_fallback_selects_logistics():
    result = main.orchestrator.function_agent.respond("订单 53cdb2fc8bc7dce0b6741e2150273451 的物流状态是什么？")
